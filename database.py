...
def save_deposit_balance(create, balance):

    user_deposit_data = first_name + "," + last_name + "," + email + "," + password + balance

    try: 

        f = open(user_db_path + str(create) + ".txt", "x")
        ...
        f.close()
# the create function is already being used in the register function
# create a different function
# i can reuse the create function for deposit and withdrawl 
# the new function has to take the account balance (parameter) 

# also create a function that tracks when the user logged in (time) 
...
def authenticated_user(account_number, password): # is this pulling from the auth.py file? in the register function?
